describe('Basic user flow for Website', () => {
  ...
  beforeAll(async () => {
    await page.goto('https://cse110-sp25.github.io/CSE110-Shop/', { waitUntil: ['domcontentloaded', 'networkidle0'] });
    // Reset test state so reruns are deterministic.
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      ...
      if (item.price === null || item.price === undefined) { allArePopulated = false; }
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const buttonText = await page.$eval('product-item', (product) => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allButtonsAreRemove = await page.$$eval('product-item', (productItems) => {
      return productItems.every((item) => {
        const button = item.shadowRoot.querySelector('button');
        return button.innerText === 'Remove from Cart';
      });
    });
    expect(allButtonsAreRemove).toBe(true);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const allButtonsAreAdd = await page.$$eval('product-item', (productItems) => {
      return productItems.every((item) => {
        const button = item.shadowRoot.querySelector('button');
        return button.innerText === 'Add to Cart';
      });
    });
    expect(allButtonsAreAdd).toBe(true);
